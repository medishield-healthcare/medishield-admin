...
function BannerFormDialog({
  ...
  const form = useForm<BannerValues>({
    ...
    defaultValues: { title: "", mobile_image: "", id: "", mobile: false },
  });
  ...
  async function handleImageChange(nextFile?: File) {
    ...
    try {
      ...
      form.setValue("mobile_image", result.url, {
        shouldDirty: true,
        shouldValidate: true,
      });
    } catch {
  ...
  async function handleSave(values: BannerValues) {
    ...
    try {
      ...
      toast.success(
        banner ? "Banner updated successfully." : "Banner added successfully.",
      );
      setOpen(false);
      await onSaved();
    } catch (error) {
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            {banner
              ? "Update the banner details or choose a replacement image."
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex flex-row items-center gap-2 space-y-0">
                  ...
                  <FormLabel className="cursor-pointer">Mobile Only</FormLabel>
  ...
  return (
    <div className="mx-auto w-full max-w-screen-2xl space-y-6 p-4 sm:p-6 lg:p-8">
      <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          <p className="mb-2 text-sm font-medium text-primary">Storefront</p>
          <h1 className="text-2xl font-semibold tracking-tight sm:text-3xl">
            Promotion Banners
          </h1>
          <p className="mt-2 text-sm leading-6 text-muted-foreground">
            Keep your featured products and promotions up to date.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-3">
        {!loading && banners.length !== 0 ? (
          banners.map((banner) => (
            <div
              key={banner._id}
              className="overflow-hidden rounded-lg border bg-card shadow-sm transition-colors hover:border-primary/30"
            >
              <img
                alt={banner.title}
                className="aspect-[2/1] w-full border-b bg-muted object-cover"
                height={150}
          ...
          <div className="col-span-full rounded-lg border border-dashed bg-card px-6 py-16 text-center text-sm text-muted-foreground">
            {loading
              ? "Loading banners..."
              : "No banners found. Add a banner to feature a promotion."}
          </div>
        )}
      </div>
    </div>
